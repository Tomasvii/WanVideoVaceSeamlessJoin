...
class CombineVideoClips:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "sixth_joined_video_path": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "display": "text",
                    "tooltip": "Path to the sixth joined video file (optional) - can be connected from other nodes"
                }),
                "seventh_joined_video_path": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "display": "text",
                    "tooltip": "Path to the seventh joined video file (optional) - can be connected from other nodes"
                }),
                "eighth_joined_video_path": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "display": "text",
                    "tooltip": "Path to the eighth joined video file (optional) - can be connected from other nodes"
                }),
                "ninth_joined_video_path": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "display": "text",
                    "tooltip": "Path to the ninth joined video file (optional) - can be connected from other nodes"
                }),
                "tenth_joined_video_path": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "display": "text",
                    "tooltip": "Path to the tenth joined video file (optional) - can be connected from other nodes"
                }),
                "eleventh_joined_video_path": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "display": "text",
                    "tooltip": "Path to the eleventh joined video file (optional) - can be connected from other nodes"
                }),
                "twelfth_joined_video_path": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "display": "text",
                    "tooltip": "Path to the twelfth joined video file (optional) - can be connected from other nodes"
                }),
    ...
    def combine_videos(self, frame_load_cap, mask_last_frames, mask_first_frames,
                      ...
                      sixth_joined_video_path=None, seventh_joined_video_path=None, eighth_joined_video_path=None,
                      ninth_joined_video_path=None, tenth_joined_video_path=None, eleventh_joined_video_path=None,
                      twelfth_joined_video_path=None, last_video_path=None):
        ...
        print(f"  sixth_joined_video_path: {sixth_joined_video_path}")
        print(f"  seventh_joined_video_path: {seventh_joined_video_path}")
        print(f"  eighth_joined_video_path: {eighth_joined_video_path}")
        print(f"  ninth_joined_video_path: {ninth_joined_video_path}")
        print(f"  tenth_joined_video_path: {tenth_joined_video_path}")
        print(f"  eleventh_joined_video_path: {eleventh_joined_video_path}")
        print(f"  twelfth_joined_video_path: {twelfth_joined_video_path}")
        ...
        if sixth_joined_video_path is None:
            sixth_joined_video_path = ""
        if seventh_joined_video_path is None:
            seventh_joined_video_path = ""
        if eighth_joined_video_path is None:
            eighth_joined_video_path = ""
        if ninth_joined_video_path is None:
            ninth_joined_video_path = ""
        if tenth_joined_video_path is None:
            tenth_joined_video_path = ""
        if eleventh_joined_video_path is None:
            eleventh_joined_video_path = ""
        if twelfth_joined_video_path is None:
            twelfth_joined_video_path = ""
        ...
        sixth_joined_video_path = str(sixth_joined_video_path).strip()
        seventh_joined_video_path = str(seventh_joined_video_path).strip()
        eighth_joined_video_path = str(eighth_joined_video_path).strip()
        ninth_joined_video_path = str(ninth_joined_video_path).strip()
        tenth_joined_video_path = str(tenth_joined_video_path).strip()
        eleventh_joined_video_path = str(eleventh_joined_video_path).strip()
        twelfth_joined_video_path = str(twelfth_joined_video_path).strip()
        ...
        try:
            ...
            sixth_joined_images_list = []
            if sixth_joined_video_path and os.path.exists(sixth_joined_video_path):
                sixth_joined_images_list = self.load_video_frames(sixth_joined_video_path)

            seventh_joined_images_list = []
            if seventh_joined_video_path and os.path.exists(seventh_joined_video_path):
                seventh_joined_images_list = self.load_video_frames(seventh_joined_video_path)

            eighth_joined_images_list = []
            if eighth_joined_video_path and os.path.exists(eighth_joined_video_path):
                eighth_joined_images_list = self.load_video_frames(eighth_joined_video_path)

            ninth_joined_images_list = []
            if ninth_joined_video_path and os.path.exists(ninth_joined_video_path):
                ninth_joined_images_list = self.load_video_frames(ninth_joined_video_path)

            tenth_joined_images_list = []
            if tenth_joined_video_path and os.path.exists(tenth_joined_video_path):
                tenth_joined_images_list = self.load_video_frames(tenth_joined_video_path)

            eleventh_joined_images_list = []
            if eleventh_joined_video_path and os.path.exists(eleventh_joined_video_path):
                eleventh_joined_images_list = self.load_video_frames(eleventh_joined_video_path)

            twelfth_joined_images_list = []
            if twelfth_joined_video_path and os.path.exists(twelfth_joined_video_path):
                twelfth_joined_images_list = self.load_video_frames(twelfth_joined_video_path)
            ...
            print(f"  Sixth joined: {len(sixth_joined_images_list)} frames")
            print(f"  Seventh joined: {len(seventh_joined_images_list)} frames")
            print(f"  Eighth joined: {len(eighth_joined_images_list)} frames")
            print(f"  Ninth joined: {len(ninth_joined_images_list)} frames")
            print(f"  Tenth joined: {len(tenth_joined_images_list)} frames")
            print(f"  Eleventh joined: {len(eleventh_joined_images_list)} frames")
            print(f"  Twelfth joined: {len(twelfth_joined_images_list)} frames")
        ...
        # o: If sixth_joined_images_list is not empty, append all its images
        if sixth_joined_images_list:
            print(f"[CombineVideo] Adding sixth joined video frames: {len(sixth_joined_images_list)}")
            output_images_list.extend(sixth_joined_images_list)

        # p: If seventh_joined_images_list is not empty, append all its images
        if seventh_joined_images_list:
            print(f"[CombineVideo] Adding seventh joined video frames: {len(seventh_joined_images_list)}")
            output_images_list.extend(seventh_joined_images_list)

        # q: If eighth_joined_images_list is not empty, append all its images
        if eighth_joined_images_list:
            print(f"[CombineVideo] Adding eighth joined video frames: {len(eighth_joined_images_list)}")
            output_images_list.extend(eighth_joined_images_list)

        # r: If ninth_joined_images_list is not empty, append all its images
        if ninth_joined_images_list:
            print(f"[CombineVideo] Adding ninth joined video frames: {len(ninth_joined_images_list)}")
            output_images_list.extend(ninth_joined_images_list)

        # s: If tenth_joined_images_list is not empty, append all its images
        if tenth_joined_images_list:
            print(f"[CombineVideo] Adding tenth joined video frames: {len(tenth_joined_images_list)}")
            output_images_list.extend(tenth_joined_images_list)

        # t: If eleventh_joined_images_list is not empty, append all its images
        if eleventh_joined_images_list:
            print(f"[CombineVideo] Adding eleventh joined video frames: {len(eleventh_joined_images_list)}")
            output_images_list.extend(eleventh_joined_images_list)

        # u: If twelfth_joined_images_list is not empty, append all its images
        if twelfth_joined_images_list:
            print(f"[CombineVideo] Adding twelfth joined video frames: {len(twelfth_joined_images_list)}")
            output_images_list.extend(twelfth_joined_images_list)

        # v: Calculate frame_load_cap // 2 and store in final_images_start_index
        ...
        # w: Append images from final_images_list starting from final_images_start_index to end
